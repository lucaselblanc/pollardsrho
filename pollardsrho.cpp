...
std::string uint_256_to_hex(const uint256_t& value) {
...
class PrivateKeyGen {
    ...
    public: PrivateKeyGen(uint256_t min_scalar, uint256_t max_scalar) : gen(std::random_device{}()),
...
void uint256_to_mpz(mpz_t private_key, boost::multiprecision::uint256_t value) {
        ...
        try {
            while (search_in_progress.load()) {
                ...
                for (int i = 0; i < hares; ++i) {
                    ...
                    if (test_mode)
                    {
                        /*-- Warning, prone to infinite loops/cycles! --*/
